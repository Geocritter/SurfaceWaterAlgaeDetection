...
var addNDVI1 = function(image) {
    ...
    var scaledNDVI = ndvi.multiply(1000).toInt16()
    return(image.addBands(scaledNDVI));
}
...
var importImage = function(sDate, eDate, roi) {
  ...
  var l8 = ee.ImageCollection("LANDSAT/LC08/C01/T1_SR")
      ...
      .filterDate(sDate, eDate)
      .filterMetadata('CLOUD_COVER','less_than',75);
  var ea5 = ee.ImageCollection("ECMWF/ERA5/MONTHLY")
      .filterBounds(roi)
      .filterDate(sDate, eDate);
...
var roi = ee.FeatureCollection('users/tonywangs/Erie_Shoreline')
var counter = 0
// ---
...
for (var i = base; i <= cap; i++) {
    ...
    if ((i-base)%5 === 0) {
      ...
      greenest = ee.Image(greenest.clip(roi))
      print(greenest)
...
for (var i = base; i <= cap; i++) {
    ...
    if ((i-base)%5 === 0) {
      ...
      //print('Scale in meters:', exportedImg.projection().nominalScale());
      var exportedImg = ee.Image(greenest
        .select(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B10', 'B11', 'NDVI'])
        .clip(roi))
        //.select(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10', 'B11', 'BQA', 'NDVI', 'year', 'month', 'day']))
...
for (var i = base; i <= cap; i++) {
    sDate = i.toString()+start
    eDate = i.toString()+end
    var Landsat7Collection = importImage(sDate, eDate, roi).ea5
    ...
    //print(imageCol[counter])
    if ((i-base)%5 === 0) {
      var name2 = (i-4)+'-'+i+'_'+'EA5'
      print(name2)
      // create an NDVI mask for each collection
      var temp = imageCol[counter]
      
      var onlyMax = temp.max();
      print(withNDVI)
      // create greenest pixel composite
      var greenest = ee.Image(onlyMax.clip(roi))
      print(greenest)
  
      // Display the result
      var display = {bands: ['maximum_2m_air_temperature']}
      Map.addLayer(greenest, display, name2)
